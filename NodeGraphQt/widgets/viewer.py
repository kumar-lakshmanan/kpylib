...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().formats()[0] == "application/x-qabstractitemmodeldatalist":
            self.custom_data_dropped.emit(
                event, QtCore.QPoint(int(pos.x()), int(pos.y()))
            )
        else:
    ...
    def dragEnterEvent(self, event):
        is_acceptable = any([
            ...
            ['nodegraphqt/nodes', 'text/plain', 'text/uri-list', 'application/x-qabstractitemmodeldatalist']
        ])
        if is_acceptable:
            event.accept()
        else:
